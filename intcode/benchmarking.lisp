...
(defmacro benchmark (rom-string title inputs)
    (let* ((rom-list
            (mapcar
                #'parse-integer
                (uiop:split-string
                    (eval rom-string)
                    :separator ",")))
           (rom-vector (make-array
                (length rom-list)
                :initial-contents rom-list
                :adjustable t)))
    `(progn
        (format t "## ~a: ##~%" ,title)
        (mapcar
            (lambda (input)
                (let ((cpu (make-cpu ,rom-vector))
                      (output-stack '()))
                    (format t "Input:  ~a~%" input)
                    (time
                        (let ((result (cpu-run-until cpu '(:input :output))))
                            (loop
                                while (or
                                    (and (eql (car result) :input) input)
                                    (eql (car result) :output))
                                when (eql (car result) :input)
                                do (progn
                                    (cpu-poke-input cpu (car input))
                                    (setf input (cdr input)))
                                when (eql (car result) :output)
                                do (push (cpu-peek-output cpu) output-stack)
                                do (setf result (cpu-run-until cpu '(:input :output))))))
                    (format t
                        "Output: ~a~%===~%"
                        (reverse output-stack))))
            ,inputs))))

(benchmark *sum-of-primes* "Sum of Primes" '((10) (1000) (100000)))
;     10 =>  0.000s
;   1000 =>  0.016s
; 100000 =>  3.249s

(benchmark *ackermann* "Ackermann Function" '((3 6)))
; (3 6) => 0.977s

(benchmark *isqrt* "Integer Square Root" '((130)))
; 130 => 0.001s

(benchmark *divmod* "Div Mod" '((1024 3)))
; (1024 3) => 0.004s

(benchmark *factor* "Prime Factorization" '((19338240) (2147483647)))
;   19338240 =>  0.459s
; 2147483647 => 10.849s
